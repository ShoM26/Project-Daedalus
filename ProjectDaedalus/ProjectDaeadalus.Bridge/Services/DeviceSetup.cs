    ...
    public class DeviceSetup
    {
        ...
        private static async Task<BridgeConfig> RunSetupServerAsync()
        {
            ...
            while (true)
            {
                ...
                // Handle CORS (Allow your Frontend to talk to Localhost)
                ...
                BridgeConfig incomingData = null;
                try
                {
                    incomingData = JsonSerializer.Deserialize<BridgeConfig>(jsonBody);
                }
                catch
                {
                }
                ...
                {
                    Console.WriteLine("[Setup] Received request without Token. Ignoring.");
                ...
                var finalConfig = new BridgeConfig
                {
                    ...
                    // Use the incoming API URL, or default to localhost
                ...
                byte[] buffer = System.Text.Encoding.UTF8.GetBytes("{\"status\":\"configured\"}");
